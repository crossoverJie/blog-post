...
前段时间在某个第三方平台看到我协作次数居然突破了 10W 字，难以想象高中 800 字作文我都得巧妙的**利用换行**来完成(懂的人肯定也干过😏)。

干了这行养成了一个习惯：能撸码验证的事情都自己验证一下。
...
传入需要扫描的文章目录即可输入结果（目前只支持 `.md` 结尾 `Markdown` 文件）

![](https://ws3.sinaimg.cn/large/006tNbRwly1fwo7xl25qnj311x03sdpa.jpg)

当然结果看个乐就行（40 几万字），因为早起的博客我喜欢大篇的贴代码，还有一些英文单词也没有过滤，所以导致结果变差较大。

> 如果仅仅只是中文文字统计肯定是准的，并且该工具内置灵活的扩展方式，使用者可以自定义过滤策略，具体请看后文。
...
为此我想基于这个简单的工具为这类朋友带来一个可实践、易理解的多线程实践。
