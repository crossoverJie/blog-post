...
我觉得大部分人直接去看 `java.util.concurrent.ThreadPoolExecutor` 的源码时都是看一个大概，因为其中涉及到了许多细节处理（位运算），还有部分 AQS 的内容，所以想要理清楚具体细节并不是那么容易。

<!--more-->

预期挨个分析源码不如自己实现一个简版，当然简版并不意味着功能缺失，需要保证核心逻辑一致。
...
`workers` 是最终存放线程池中运行的线程池，在 `j.u.c` 源码中是一个 `HashSet` 所以对他所有的操作都是需要加锁。
...
当然我们不能无限制的创建线程，不然拿线程池来就没任何意义了。于是 `miniSize maxSize` 这两个参数才有它的意义。

但这两个参数再哪一步的时候才起到意义呢？这就是首先需要明确的。
...
以上的步骤和刚才那张流程图是一样的，这样看来大家是否有看出什么坑嘛？
...
- 第一步是将创建线程时传过来的任务执行（`task.run`）,接着会一直不停的从队列里获取任务执行，直到获取不到任务了。
...
- 当线程数超过核心线程数时，在获取任务的时候需要通过保活时间从队列里获取任务；一旦获取不到任务则队列肯定是空的，这样返回 `null` 之后在上文的 `run()` 中就会退出这个线程；从而达到了回收的目的，也就是我们之前演示的效果
![](http://ww3.sinaimg.cn/large/006tNc79ly1g35zpt0t7zj30qo06lwg9.jpg)
- 这里需要加锁，加锁的原因这里肯定会出现并发情况，不加锁会导致 `workers.size() > miniSize` 条件多次执行。
...
还是以刚才那段测试代码为例,如果提交任务后我们没有关闭线程，会发现即便是任务执行完毕也会程序也不会退出。

从刚才的源码里其实也很容易看出来了，不退出的原因是 Worker 线程一定还会一直阻塞在 `task = workQueue.take();`，即便是线程缩容了也不会小于核心线程数。
...
上文其实也或多或少提到了线程回收的事情，其实总结就是以下两点：
